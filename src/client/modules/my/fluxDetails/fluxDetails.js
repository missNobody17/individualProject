...
export default class FluxDetails extends LightningElement {
    ...
    async connectedCallback() {
        const [data, dLengths] = await fetch(
            `/electrons?month=${this.month}&year=${this.year}&day=${this.day_val}&isDay=${this.is_day_val}&station=${this.station}`
        )
        ...
        const data2 = await fetch(
            `/protons?month=${this.month}&year=${this.year}&day=${this.day_val}&isDay=${this.is_day_val}&station=${this.station}`
        )
            .then(function (response) {
                return response.json();
            })
    ...
    handleEarthquake() {
        if (this.is_day_val !== 'night') {
            let index = Boolean(this.day_val) ? (this.earthquake_val.getHours() - 2) * 12 : (this.earthquake_val.getDate()) * 288;
    ...
    sendFlux() {
        const selectedEvent = new CustomEvent('sendflux', {
            detail: { protons: this.protons, electrons: this.electrons }
        });
